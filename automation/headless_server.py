...
def pitch_shift_workflow(input_path, semitones, formant_ratio=1.0, vocal_gain_db=None):
    ...
    try:
        ...
        if gain_db is None:
            ...
            # mặc định: chỉ NÂNG giọng lên bằng beat khi giọng nhỏ hơn;
            # giọng đã to hơn beat thì giữ nguyên, không hạ xuống
            gain_db = max(0.0, min(15.0, im - vm)) if vm is not None and im is not None else 0.0
